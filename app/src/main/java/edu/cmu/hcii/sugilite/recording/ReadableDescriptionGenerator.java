...
public class ReadableDescriptionGenerator {
    ...
    public String generateReadableDescription(SugiliteBlock block){
        ...
        if(block instanceof SugiliteOperationBlock){
            ...
            // crucial step detection code

            if (((SugiliteCrucialOperation)((SugiliteOperationBlock) block).getOperation()).getIsCrucial() ) {
                message += setColor("\nNOTE: this step was detected as a crucial step due to the keyword \"" + ((SugiliteCrucialOperation) ((SugiliteOperationBlock) block).getOperation()).getCrucialKeyword() + ".\"", Const.CRUCIAL_STEP_COLOR);
            }
